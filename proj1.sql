...
m.EXP,COUNT(e.EMP_ID) as "EMP_COUNT" FROM emp_record_table m INNER JOIN emp_record_table e
ON m.EMP_ID = e.MANAGER_ID
GROUP BY m.EMP_ID ORDER BY m.EMP_ID;
...
OVER(PARTITION BY m.DEPT) AS "MAX_DEPT_RATING" FROM emp_record_table m ORDER BY DEPT;
...
RANK() OVER(ORDER BY EXP DESC) EXP_RANK FROM emp_record_table;
...
set salary=(select salary +(select salary*.05*EMP_RATING));
